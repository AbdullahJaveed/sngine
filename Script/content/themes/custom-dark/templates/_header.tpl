{if !$user->_logged_in}
  ...
  <body data-hash-tok="{$session_hash['token']}" data-hash-pos="{$session_hash['position']}" {if $system['theme_mode_night']}data-bs-theme="dark" {/if} class="{if $system['theme_mode_night']}night-mode{/if} visitor n_chat {if $page == 'index' && !$system['newsfeed_public']}index-body{/if} {if $system['theme'] == 'custom-dark'}theme-custom-dark{/if}" {if $page == 'profile' && $system['system_profile_background_enabled'] && $profile['user_profile_background']}style="background: url({$profile['user_profile_background']}) fixed !important; background-size: 100% auto;" {/if}>
    <!-- Inject deferred theme stylesheet + noscript fallback -->
    <link rel="preload" href="{$system['system_url']}/content/themes/{$system['theme']}/css/theme.min.css" as="style" onload="this.onload=null;this.rel='stylesheet'">
    <noscript><link rel="stylesheet" href="{$system['system_url']}/content/themes/{$system['theme']}/css/theme.min.css"></noscript>

  {else}

  <body data-hash-tok="{$session_hash['token']}" data-hash-pos="{$session_hash['position']}" data-chat-enabled="{$user->_data['user_chat_enabled']}" {if $system['theme_mode_night']}data-bs-theme="dark" {/if} class="{if $system['theme_mode_night']}night-mode{/if} {if !$system['chat_enabled'] || $user->_data['user_privacy_chat'] == "me"}n_chat{/if}{if $system['activation_enabled'] && !$system['activation_required'] && !$user->_data['user_activated']} n_activated{/if}{if !$system['system_live']} n_live{/if} {if $system['theme'] == 'custom-dark'}theme-custom-dark{/if}" {if $page == 'profile' && $system['system_profile_background_enabled'] && $profile['user_profile_background']}style="background: url({$profile['user_profile_background']}) fixed !important; background-size: 100% auto;" {/if} {if $url}onload="initialize_scraper()" {/if}>
    <!-- Inject deferred theme stylesheet + noscript fallback -->
    <link rel="preload" href="{$system['system_url']}/content/themes/{$system['theme']}/css/theme.min.css" as="style" onload="this.onload=null;this.rel='stylesheet'">
    <noscript><link rel="stylesheet" href="{$system['system_url']}/content/themes/{$system['theme']}/css/theme.min.css"></noscript>
              ...
              <div class="col-7 col-md-4 col-lg-3">
                ...
                <div class="logo-wrapper" {if !($user->_logged_in || (!$user->_logged_in && $system['newsfeed_public']))}style="padding-left: 0;" {/if}>
                  ...
                  {if $user->_logged_in || (!$user->_logged_in && $system['newsfeed_public'])}
                    <!-- menu-icon -->
                    <a href="#" data-bs-toggle="sg-offcanvas" class="menu-icon d-block d-md-none" role="button" aria-label="Open navigation" aria-expanded="false">
                  ...
                  <a href="{$system['system_url']}" class="logo">
                    {if $system['system_logo']}
                      <img class="logo-light img-fluid" src="{$system['system_uploads']}/{$system['system_logo']}" alt="{__($system['system_title'])}" title="{__($system['system_title'])}" loading="lazy" decoding="async">
                      {if !$system['system_logo_dark']}
                        <img class="logo-dark img-fluid" src="{$system['system_uploads']}/{$system['system_logo']}" alt="{__($system['system_title'])}" title="{__($system['system_title'])}" loading="lazy" decoding="async">
                      {else}
                        <img class="logo-dark img-fluid" src="{$system['system_uploads']}/{$system['system_logo_dark']}" alt="{$system['system_title']}" title="{__($system['system_title'])}" loading="lazy" decoding="async">
                      {/if}
                    {else}
              ...
              <div class="col-5 col-md-8 col-lg-9">
                <div class="row">
                  ...
                  <div class="col-md-5 col-lg-4">
                    <!-- navbar-wrapper -->
                    <div class="navbar-wrapper touch-nav">
                      <ul>
                        {if $user->_logged_in}
                        ...
                        {else}
                          ...
                          <li class="dropdown">
                            <a href="#" class="dropdown-toggle user-menu float-end" data-bs-toggle="dropdown" data-display="static">
                              <img src="{$system['system_url']}/content/themes/{$system['theme']}/images/blank_profile_male.png" loading="lazy" decoding="async">
                              <span>{__("Join")}</span>
                            </a>
                            <div class="dropdown-menu dropdown-menu-end">
